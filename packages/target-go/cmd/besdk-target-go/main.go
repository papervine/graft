// Command besdk-target-go is the besdk Go target.
//
//	besdk-target-go --sdk-target-protocol   → handshake on stdout
//	besdk-target-go                    → IR JSON on stdin, file manifest on stdout
//
// Written in Go so it can use Go's own tooling — go/format for layout, go build and go vet as gates.
// Nothing here imports anything from besdk's core: the contract is the JSON, which is what makes a
...
import (
	...
	"github.com/besdk/target-go/internal/emit"
)
...
// package that its own test suite exercises. Tests are excluded: they belong to besdk's repository,
// not to a user's SDK.
...
func run() int {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "besdk-target-go: reading stdin: %v\n", err)
		return 2
	}
	if len(strings.TrimSpace(string(raw))) == 0 {
		fmt.Fprintln(os.Stderr, "besdk-target-go: expected IR JSON on stdin")
		return 2
	}

	var input emit.TargetInput
	if err := json.Unmarshal(raw, &input); err != nil {
		fmt.Fprintf(os.Stderr, "besdk-target-go: stdin was not valid JSON: %v\n", err)
		return 2
	}
	...
	if problems := emit.Sanity(&input.IR, loose.IR); len(problems) > 0 {
		fmt.Fprintln(os.Stderr, "besdk-target-go: the IR did not decode as expected:")
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "besdk-target-go: %v\n", err)
		return 2
	}
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "besdk-target-go: encoding manifest: %v\n", err)
		return 70
	}
	fmt.Println(string(encoded))
	return 0
