 ...
 *
 * It was two. `generate` honoured `targets.<name>.command`, an installed `@graft/target-<name>`
 ...
 * project, and the first thing anyone evaluating graft would run.
 *
...
import { BRAND, TARGET_EXECUTABLE_PREFIX } from '@graft/protocol';
 ...
 * 2. **An installed `@graft/target-<name>` package** — so a checkout works with no global install.
 * 3. **`graft-target-<name>` on `PATH`** — how every third-party target joins.
 *
...
export function resolveTarget(
  ...
  try {
    const manifestPath = require.resolve(`@graft/target-${name}/package.json`);
