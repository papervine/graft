"""The target protocol entry point (SPEC.md §3.5).
    ...
    besdk-target-python --sdk-target-protocol   → handshake on stdout
    besdk-target-python                         → IR JSON on stdin, manifest on stdout

This is the first target that is not a Node program, which is the whole reason the protocol is a
subprocess boundary rather than a plugin API. Nothing here imports anything from besdk's core: the
...
def _gates() -> list[dict[str, object]]:
    ...
    Declared by the target rather than hardcoded in besdk's core: only the target knows that Python
...
def load_runtime_sources() -> dict[str, str]:
    ...
    normal library that its own test suite exercises. Tests are excluded: they belong to besdk's
    repository, not to a user's SDK.
    """
    here = Path(__file__).resolve()
    candidates = [
        here.parents[3] / "runtime-python" / "src" / "besdk_runtime",
        here.parents[2] / "besdk_runtime",
    ]
...
def main(argv: list[str]) -> int:
    ...
    raw = sys.stdin.read()
    if raw.strip() == "":
        sys.stderr.write("besdk-target-python: expected IR JSON on stdin\n")
        return 2

    try:
        payload = json.loads(raw)
    except json.JSONDecodeError as error:
        sys.stderr.write(f"besdk-target-python: stdin was not valid JSON: {error}\n")
        return 2

    ir = payload.get("ir")
    if not isinstance(ir, dict):
        sys.stderr.write("besdk-target-python: input had no `ir` object\n")
        return 2
...
def cli() -> None:
    """Console-script entry point, so `besdk-target-python` works when installed on PATH."""
