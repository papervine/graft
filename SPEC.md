...
`graft` exists to remove that risk without accepting the usual open-source quality penalty.
...
2. Under-specification must be **surfaced, never silently papered over.** A generator that
   ...
   is useless. Hence `graft check` (§3.6) is a first-class command, not a lint afterthought.
...
flag, a `graft.yaml` key, an `x-graft-*` extension, a diagnostic code, or the shape of generated
output.
...
**`./docs` is organised by what a user is trying to do, not by how graft is built.** Two groups do the
...
`graft` is provisional, so that cost is paid up front instead.
...
Not centralized, deliberately: **workspace package names** (`@graft/*`) and import specifiers are
...
`"Code generated by graft. DO NOT EDIT."`, landing the name in the one place it must never appear.
...
#### 1.2.1 The first rename: `besdk` → `graft`

**Decision: the project is called `graft`.** The previous working title, `besdk`, was a placeholder
that described a category rather than naming a thing; `graft` names what the tool does — joining
generated tissue onto a living codebase so it takes rather than sits beside it.

The rename is recorded here because it is the first exercise of the machinery above, and what it
found is worth more than the outcome:

- **The constant did its job, and the mechanical pass was the whole cost.** `BRAND_NAME` covered the
  CLI binary, `graft.yaml`, `.graft/`, `x-graft-*`, `graft-target-*`, help text, diagnostics, and
  generated attribution. Everything else was the *known* uncentralizable set: workspace package
  names (`@besdk/*` → `@graft/*`), the Python distributions (`besdk_runtime`, `besdk_target_python`),
  the Go module paths, the Java packages and their directories, the PHP namespaces, and the C#
  assemblies. That set is exactly what §1.2 predicted it would be, which is the evidence that the
  boundary was drawn in the right place.
- **The guard tests passed unchanged.** No string literal had to be exempted, and no generated
  artifact under `sdks/` named the generator — confirming that `SDKError` being role-named is load
  bearing rather than decorative.
- **The three things a rename breaks are all build caches, not source.** Composer's PSR-4
  autoloader, the .NET `obj/` restore graph, and pnpm's `node_modules` links each keep the old name
  and fail confusingly. They regenerate, but a future rename should regenerate them deliberately
  rather than discovering it through a test failure.

**`LEGACY_EXTENSION_PREFIXES` stays empty.** `besdk` never shipped to a registry, so no spec in the
wild carries `x-besdk-*`, and adding a prefix nobody used would be dead code that implies a
compatibility promise this project never made. The list exists for the *next* rename, if there is
one after a release.

**Decision: `BRAND_TITLE` is no longer identical to `BRAND_NAME`.** It is `Graft`, title-cased, while
`BRAND_NAME` stays lowercase `graft` for the binary and every derived filename. Under `besdk` the two
were the same string, which made the distinction look redundant; a name with a natural capitalized
form shows why it is not. Prose and generated attribution read `Graft`; anything a user types reads
`graft`.
...
**Decision: the `graft` core is written in TypeScript.** Reasoning, in the order that
actually decides it:
...
**Config overlay.** A user-supplied config file (`graft.yaml`) supplements what the spec
...
and `x-fern-sdk-method-name` carry exactly the resource/method grouping graft needs, and the
...
graft read them through the same line of code. The distinction is now carried in the IR as
...
and that is exactly the class of thing `graft check` exists to surface rather than absorb silently.
...
**Type changes in `graft diff` were unconditionally breaking**, which is the same mistake required-ness
...
| Synthesized names never shadow a JS/DOM global | a model called `Error` or `Response` shadows the built-in for any module importing it, and fails confusingly rather than loudly. Applies only to names *graft* chooses — a component the spec itself names `Event` keeps that name |
...
- **`targets.<name>.command`**, an argv escape hatch. §3.5 already claimed targets resolve "on `PATH`
  or via `graft.yaml`"; only the first half was implemented. A target written in another language is
...
**Decision: the target is launched through a committed wrapper script, `bin/graft-target-dotnet`, and the
package's `.gitignore` is scoped to `src/bin/` rather than a bare `bin/`.** Both halves were bugs found by
deleting the build tree during the `graft` rename (§1.2.1).

`dotnet build` produces an *apphost* — a native launcher that still needs the shared .NET runtime — so
naming it directly in `graft.yaml` fails with `You must install .NET to run this application` on a machine
whose runtime comes from devbox rather than a system install. That reads as a missing dependency rather than
a missing `PATH` entry, which is the wrong diagnosis to hand someone. The wrapper resolves an SDK the way the
suites do (`SDK_DOTNET`, then devbox, then `PATH`) and runs the assembly through it, exactly as the Java
target's wrapper resolves a JVM new enough to load its own class files.

The `.gitignore` half is the more instructive failure. `bin/` is the obvious MSBuild ignore, and it also
swallowed the launcher — so the file existed on the machine that wrote it, was never committed, and a fresh
clone could build this target and still not run it. **A pattern that matches build output by name rather than
by location will eventually match something hand-written**, and the symptom appears only on a machine that
has never built locally. Scoping the ignore to the project directory it describes is the general fix.
...
override in `graft.yaml` would be more direct and is not yet built.
...
replay of non-idempotent requests", which sounds like an enhancement. But graft already retried, and the
...
graft generates for *arbitrary* APIs. Sending `Idempotency-Key` to a server that ignores it produces a
...
`graft.yaml` names the header because it is not standardised (`Idempotency-Key`, `X-Idempotency-Key`,
...
graft modelled none of it, so the base URL kept its braces and every request went to a host that does not
...
their own code. `validation: warn` and `validation: off` exist for a sloppy server, and `graft check`
...
method, and `Last-Event-ID` is a request option. graft does not reconnect.**
...
**graft does not reconnect, and that is the scoping decision.** Automatic resumption sounds like the
...
So graft supplies everything resumption needs — the id on each event, and `RequestOptions.lastEventId` to
...
*Rejected alternative:* reconnecting behind an opt-in flag. It would still be graft asserting that
...
tsconfig is theirs, and generated code that only compiles under the settings graft happens to emit is
...
`graft.yaml` key a user can set and graft ignores is worse than an absent feature.
...
`callbacks` on an operation, and Fern, Speakeasy, and Redocly all read `x-webhooks`. graft reads all
...
So `webhooks.signature` in `graft.yaml` names the algorithm, the header, how to get the signature out of
...
signature scheme, so graft cannot infer one. A `verify()` that returns `true` because it checked
...
fails in the direction of accepting forged requests, and graft has no way to notice. The descriptor is
...
it composes with Express, FastAPI, `net/http`, or a Lambda handler without graft knowing about any of
...
**A note on how the gap was measured, which is a lesson about verification rather than about graft.** The
...
```
graft-target-<lang> --sdk-target-protocol
...
targets ship in-tree. Third-party targets resolve as `graft-target-*` on `PATH` or via
`graft.yaml`.
...
**`graft targets` and `graft generate` resolved targets differently.** `generate` honoured
...
in the first thing anyone evaluating graft would run. One resolver now, in
...
tested. `graft targets` prints that list, so an author reading the reference target to learn what is
...
vocabulary; there is no way to infer the boundary otherwise. graft ships a curated word list and
`naming.words` in `graft.yaml` extends it.
...
Humans name a type after what it *is* and add parent context only to disambiguate, so graft tries
...
`naming.words` extends the vocabulary, so a user can fix a domain word without patching graft.

**Synthesized names never shadow a JS/DOM global.** A model called `Error` or `Response` shadows
the built-in for any importing module and fails confusingly. Applies only to names graft chooses
...
**Decision: graft defines `x-graft-*`, and also reads other generators' extensions.**
...
not stop being true because it was written for a different tool. Ignoring it would make graft
...
|---|---|---|
| 1 | `graft.yaml` | the consumer's explicit override, closest to whoever runs graft |
...
Supported `x-graft-*`: `group` and `method` on an operation (dotted group nests), `ignore` on an
...
express: it is precisely what graft cannot infer, the API owner knows it for certain, and living
...
**An extension graft half-understands is worse than one it ignores**, because the user assumes it
...
too, since a typo in `x-graft-server-owned` would otherwise be indistinguishable from silence.

`graft check` reports which tier supplied each name, so a surprising method name can be traced to
its source.

#### 3.1.6 `graft.yaml` conventions
...
- **`graft init` writes inferred values explicitly**, each tagged with the diagnostic code that
  produced it, and marks with `REVIEW` the judgments only the API owner can make. A config
  that omitted everything graft could guess would leave those guesses invisible and
  un-overridable — the overlay would be magic, which is the opposite of the point.

`graft init` refuses to overwrite an existing `graft.yaml` without `--force`: the file
accumulates hand-made judgments about the API and is the most expensive artifact in the repo.

### 3.5.1 `graft release` — versions, changelogs, and publishing

**Decision: graft computes the version and the changelog; CI does the publishing.**

That split is the whole design, and both halves are deliberate.

**What graft owns, because only it can.** The next version is a *diff* question, and `graft diff` already
...
**What graft does not own, and must not.** It never runs `npm publish`, `twine upload`, or `git push
...
could not parse (§3.3.3). So the SDK version is tracked separately, in `.graft/<spec>.sdk-version`
...
**Go has no version field, and that is not a gap in graft.** A Go module's version *is* its git tag, so
...
### 3.6 `graft check` — surfacing under-specification
...
say*. Every diagnostic carries a copy-pasteable `graft.yaml` fix.
...
answered in `graft.yaml` is noise, and a `check` that keeps naming a fixed problem stops being read.
...
spec: `graft init` writes `models.Widget.split`, `models.Widget.required`, and `pagination.default`, then
...
packages/cli/                 graft binary; commands, reporters, exit codes
...
### 3.8 `graft diff` — breaking-change detection
...
- **Config overlay format: YAML** (`graft.yaml`). Matches the input format, so users edit one
  syntax; comments are essential since the file records judgment calls; and `graft init`
  writes inferred values *explicitly* so nothing about the output is magic.
- **License: MIT.** *Reversed from Apache-2.0.* The original reasoning was that Apache's explicit
  patent grant is what makes companies comfortable depending on a generator for their public SDKs —
  the adoption thesis in §1. That argument over-weighted a risk that does not really apply here:
  this project holds no patents to grant, and the artifact a company actually depends on is the
  *generated* SDK, which carries no licence from us at all (the targets stamp none — the API owner
  chooses). So the patent grant protects against a hazard that the vendoring model has already
  removed, while Apache's NOTICE and modification-marking obligations are real friction for anyone
  redistributing a fork. MIT is the licence most of the ecosystem a contributor already knows —
  including the sibling project this one shares an organisation with — and being unremarkable is
  worth more here than being maximally protective.
...
All figures below are measured by `graft check`, not estimated by hand.
...
graft. Every one of these was a real defect, and none was reachable from the specs already in the
corpus:
...
| `init` wrote a config `graft` itself rejected | GitHub's error body has an `array` field, and the config shorthand accepts only scalars |
...
(DigitalOcean's spec is split this way and graft resolves only same-document pointers), and
...
Private IR baselines are gitignored too (`.graft/corpus-private-*`), even though `.graft/` is
...
graft change, and the snapshots would be worthless as a regression signal.
...
honest, but it remains a weaker test than a spec nobody designed for graft. Acquiring a real
...
graft supports both, because a helper module and a one-method convenience wrapper are different
...
methods between them are preserved — so a user with no `preserve` block in `graft.yaml` read an
...
3. **Silent staleness is reported.** A preserved file also stops receiving improvements, so graft
...
`graft:`-prefixed. A marker carrying the tool's name would live in users' files, making a rename of
...
**Decision: graft records the files it wrote to each output directory and removes, on the next run, the
...
`graft generate <spec>` picked up an `examples/pagination.ts` left by an earlier run with a different
...
leaving a stale file is not, so the rule is narrow: a path is removable only if graft wrote it last time
...
*Rejected alternative:* deleting any file matching a pattern graft owns (`src/resources/*.ts`). Patterns
...
crashes on handshake, and one that does not accept the IR version graft emits. It also lets someone
...
conformance suite exercises the kitchen-sink spec, which declares JSON bodies. `graft check` reports what
...
3. **The differentiators stay genuinely better, not merely present.** `graft check`, direction-sensitive
   `graft diff`, reading competitors' extensions, and the no-lock-in thesis are the reasons to choose
   this over a funded product. A feature that exists but is worse than the competitor's equivalent is
   not a differentiator.
4. **Nothing under "Client-level features the competitors ship and graft does not" remains undecided.**
   Not *built* — decided. A recorded non-goal satisfies this; a blank does not. The list exists because
   the first three conditions are all measured against graft's own intentions, and a gap nobody wrote
...
1. ~~**Code preservation across regeneration.**~~ Built (§3.9): `preserve.files`, `.graftignore`, and
...
6. ~~**Publishing automation.**~~ Built as `graft release` (§3.5.1): the version comes from the
...
Fern generates nine SDK languages; graft generates six. **Decision (2026-08-06): match Fern's set, PHP
...
### Client-level features the competitors ship and graft does not

Derived by walking Stainless's and Fern's published documentation and diffing their feature inventory
against what graft *emits* — not against what this document intends, which is the comparison that flatters. Everything here is a **wire- or client-level capability**, not product
surface, so none of it is excused by the out-of-scope decision above.

**WebSocket clients.** Both competitors generate them; graft has no mention of them anywhere, in the IR
...
graft resolves auth once at construction, and the only moving credential it supports is an OAuth2
...
(`x-fern-audiences`, which graft reads and reports as unsupported). Both are the same underlying feature —
*emit a subset of the spec, chosen by annotation* — and graft has one half of it already in
`x-graft-ignore`, which is unconditional. Generalising `ignore` into a named-audience filter is a
...
**SSE metadata.** Fern exposes the event `id` and `retry` fields alongside the decoded payload. graft's
...
graft multi-targets by default or exposes `targetFrameworks` in config.

**Python is the one target with runtime dependencies**, and Fern offers `aiohttp` where graft pins `httpx`.
...
else's machine against a tracked branch. graft generates locally, from a spec on disk, into a directory
you name, every time; `--emit-stage` exposes each pipeline stage and `graft diff` shows what regenerating
would do to consumers before it does it. There is nothing to add here, and a `--preview` flag would be a
synonym for `generate`. Likewise Stainless's "AI-generated commit messages": `graft release` derives the
...
gate is verification of the SDK, not product surface around it. Designed in §3.11. Operationally: a standalone binary for air-gapped use — graft
...
not the IR — targets could not declare their own verification gates, and the documented `graft.yaml`
...
- **`graft check`** (§3.6) — surfacing under-specification appears to be genuinely novel.
- **`graft diff`** (§3.8) — direction-sensitive breaking-change classification: read and write models
...
`graft-php`** — the generator's name travelling in consumers' HTTP traffic, which §1.2 forbids outright. It
...
- **Version-bump automation.** `graft diff` (§3.8) reports the implied semver bump, but nothing
  applies it to the generated `package.json` yet. Open: whether graft should write the version,
...
- **Capabilities are declared but never enforced.** Every target declares a `capabilities` list and
  `graft targets` prints it, but nothing compares it against what the IR actually contains. A target
